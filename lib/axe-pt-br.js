axe.configure({
    locale: {
        ...
        "rules": {
            ...
            "target-size": {
                "description": "Garanta que os alvos de toque tenham tamanho e espaço suficientes",
                "help": "Todos os alvos de toque devem ter 24px de tamanho ou deixar espaço suficiente"
            },
            "target-offset": {
                "description": "Garanta que os alvos de toque tenham espaço suficiente entre eles",
                "help": "Alvos de toque devem ter espaço suficiente em relação aos seus vizinhos mais próximos"
            }
        },
        "checks": {
            "abstractrole": {
            ...
            "target-size": {
                "pass": {
                    "default": "O controle tem tamanho suficiente (${data.width}px por ${data.height}px, deve ser pelo menos ${data.minSize}px por ${data.minSize}px)",
                    "obscured": "O controle é ignorado porque está totalmente obscurecido e, portanto, não é clicável",
                    "large": "O alvo excede em muito o tamanho mínimo de ${data.minSize}px."
                },
                "fail": {
                    "default": "O alvo tem tamanho insuficiente (${data.width}px por ${data.height}px, deve ser pelo menos ${data.minSize}px por ${data.minSize}px)",
                    "partiallyObscured": "O alvo tem tamanho insuficiente porque está parcialmente obscurecido (o menor espaço é ${data.width}px por ${data.height}px, deve ser pelo menos ${data.minSize}px por ${data.minSize}px)"
                },
                "incomplete": {
                    "default": "Elemento com tabindex negativo tem tamanho insuficiente (${data.width}px por ${data.height}px, deve ser pelo menos ${data.minSize}px por ${data.minSize}px). Isso é um alvo?",
                    "contentOverflow": "O tamanho do elemento não pôde ser determinado com precisão devido ao transbordamento de conteúdo",
                    "partiallyObscured": "Elemento com tabindex negativo tem tamanho insuficiente porque está parcialmente obscurecido (o menor espaço é ${data.width}px por ${data.height}px, deve ser pelo menos ${data.minSize}px por ${data.minSize}px). Isso é um alvo?",
                    "partiallyObscuredNonTabbable": "O alvo tem tamanho insuficiente porque está parcialmente obscurecido por um vizinho com tabindex negativo (o menor espaço é ${data.width}px por ${data.height}px, deve ser pelo menos ${data.minSize}px por ${data.minSize}px). O vizinho é um alvo?",
                    "tooManyRects": "Não foi possível obter o tamanho do alvo porque há muitos elementos sobrepostos"
                }
            },
            "target-offset": {
                "pass": {
                    "default": "O alvo tem espaço suficiente de seus vizinhos mais próximos. O espaço clicável seguro tem um diâmetro de ${data.closestOffset}px que é de pelo menos ${data.minOffset}px.",
                    "large": "O alvo excede em muito o tamanho mínimo de ${data.minOffset}px."
                },
                "fail": "O alvo tem espaço insuficiente para seus vizinhos mais próximos. O espaço clicável seguro tem um diâmetro de ${data.closestOffset}px em vez de pelo menos ${data.minOffset}px.",
                "incomplete": {
                    "default": "Elemento com tabindex negativo tem espaço insuficiente para seus vizinhos mais próximos. O espaço clicável seguro tem um diâmetro de ${data.closestOffset}px em vez de pelo menos ${data.minOffset}px. Isso é um alvo?",
                    "nonTabbableNeighbor": "O alvo tem espaço insuficiente para seus vizinhos mais próximos. O espaço clicável seguro tem um diâmetro de ${data.closestOffset}px em vez de pelo menos ${data.minOffset}px. O vizinho é um alvo?",
                    "tooManyRects": "Não foi possível obter o tamanho do alvo porque há muitos elementos sobrepostos"
                }
            }
        },
